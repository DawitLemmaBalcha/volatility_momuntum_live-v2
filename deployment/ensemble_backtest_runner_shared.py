# ensemble_backtest_runner_shared.py
import warnings
...
# --- *** MODIFIED FOR MODEL C *** ---
from simulation.backtest_runner_shared import SimulationEngine
from core.trading_bot_shared import AdvancedAdaptiveGridTradingBot
# --- *** END MODIFICATION *** ---


def setup_logging():
    log_filename = f"ensemble_backtest_SHARED_log_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
...
def log_prop_firm_summary(portfolio_metrics: dict):
    if not portfolio_metrics:
        ...
        return
    logging.info("\n" + "="*25 + " CONSOLIDATED PORTFOLIO SUMMARY (MODEL C) " + "="*23)
    logging.info("   (Simulating all strategies on a single SHARED account)")
...
def run_ensemble_backtest():
    """Orchestrates a concurrent backtest for all strategies using a SHARED capital pool."""
    logging.info("--- Starting Concurrent Ensemble Backtest (SHARED CAPITAL - MODEL C) ---")
    ...
    num_bots_total = 0
    for venue_config in backtest_venues.values():
        num_bots_total += len(venue_config["strategies"])

    if num_bots_total == 0:
        logging.error("No strategies found in any 'backtest' venues. Exiting.")
        return

    allocation_per_bot = 1.0 / num_bots_total
    logging.info(f"Found {num_bots_total} strategies. Each will be allocated {allocation_per_bot:.2%} of sizing power.")
    # --- *** END NEW *** ---
    ...
    logging.info(f"2. Initializing SimulationEngine (Model C) with {len(all_bots)} bots...")
    ...
    # This logs the main portfolio performance
    log_prop_firm_summary(portfolio_results)

    # This logs the individual "hero/loser" stats
    ...
    for i, bot_metrics in enumerate(individual_results):
        ...
        # Print the detailed log, skipping the '====' borders
